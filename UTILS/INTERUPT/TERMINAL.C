...
#define VerzeProgramu     "2.1a"
#define Creator           "Tomas Hujer"
#define PosledniKompilace "6. 11. 2001"
#define ConfigFileHeader  "THS Terminal Config File "VerzeProgramu"\n"
...
unsigned char Pol,UPol,SPol,SNPol,NPol,DCPol,STPPol,DistelPol,DSPol,
              DBLPol;
unsigned int  _Byte;
...
static unsigned char DistelBuf[256];
static unsigned char BufIndex, BufPos;
static unsigned long SetTimeout;
static unsigned char DetectTimeout;
static long Timeout;
...
unsigned char FilterChar(unsigned char Ch)
{
  ...
  switch(Ch)
  {
    case 0x0D: if(Config.DisplayChars[0]) _Ch=0; else _Ch=Ch;  break;
    case 0x0A: if(Config.DisplayChars[1]) _Ch=0; else _Ch=Ch; break;
    case 0x07: if(Config.DisplayChars[2]) _Ch=0; else _Ch=Ch;  break;
    case 0x08: if(Config.DisplayChars[3]) _Ch=0; else _Ch=Ch;  break;
    case 0x09: if(Config.DisplayChars[4]) _Ch=0; else _Ch=Ch;  break;
    default: _Ch=Ch; break;
  }
...
unsigned int AdrPort(char P)
{
  if(!SetFixAdress)
  {
    switch(P)
    {
      ...
      case 2: return(peek(0,0x0404));
      case 3: return(peek(0,0x0406));
...
void ShowHint(unsigned char *Hint)
{
   ...
   if(Hint[0]!=0) { gotoxy(1,24); cprintf(" %s",Hint); }
}
...
void About()
{
  ...
  CenterText(13,"Copyright (c) 2001");
...
void ExitAbout(unsigned char ExitCode)
{
  ...
  CenterText(3,"Copyright (c) 2001  "Creator"\n\n");
...
void ReadConfigFile()
{
  ...
  if ((handle = _open("TERMINAL.CFG", O_RDONLY )) == -1)
  {
     ...
     Config.DisplayChars[0]=0;    // 0x0D  CR
     Config.DisplayChars[1]=0;    // 0x0A  LF
     Config.DisplayChars[2]=0;    // 0x07  BELL
     Config.DisplayChars[3]=0;    // 0x08  BCK
     Config.DisplayChars[4]=0;    // 0x09  TAB
     ...
     Config.DistelMaxAutoSend=0;      // Maximalni pocet odeslani
     Config.DistelMaxRecMsgs=13;      // Max. pocet prijimanych zprav
     Config.DistelTimeoutMsgDetect=50;// Timeout pro odliseni zprav
     ...
     Config.DistelMSGDetectionType=2;
...
void InitPort()
{
  ...
  {
    GetScr(2,17,11,67,14);
    Frame(18,12,66,14,64+15,0);
    gotoxy(38,12); cprintf(" Chyba ");
    gotoxy(20,13); cprintf("Port COM%x na adrese %.3Xh nelze inicializovat !",Config.Port+1,Config.AdresaPortu);
    while(1)
    {
...
{
  if((inp(Config.AdresaPortu+5)&(32+64))==(32+64)) return(1); else return(0);
}
...
{
  if((inp(Config.AdresaPortu+5)&(1))==1) return(1); else return(0);
}
...
unsigned char VysliByte(char B)
{
  ...
  while(1)
  {
    if(SendReady()) { outp(Config.AdresaPortu,B); return(0); }
    delay(1);
    TimeOut--;
...
void RefreshMainMenu()
{
  ...
  MenuItems[ 5].Y=8;  MenuItems[ 5].Item=" 2xCOM  \0x0";
  MenuItems[ 6].Y=10; MenuItems[ 6].Item=" Set   \0x0";
  MenuItems[ 7].Y=11; MenuItems[ 7].Item="Utils  \0x0";
  MenuItems[ 8].Y=13; MenuItems[ 8].Item="Distel  \0x0";
  MenuItems[ 9].Y=22; MenuItems[ 9].Item="Exit    \0x0";
  MenuItems[10].Y=9;  MenuItems[10].Item[0]=0;
  MenuItems[11].Y=12; MenuItems[11].Item[0]=0;
  MenuItems[12].Y=21; MenuItems[12].Item[0]=0;
  for(I=0;I<13;I++)
  {
    if(Pol==I) textattr(15); else textattr(7+16);
...
//------------------------------------------------------------------------
// Vykresli ramecek a prekresli hlavni menu
//------------------------------------------------------------------------
void InitMainMenu()
{
...
//------------------------------------------------------------------------
//  Vykresleni ramecku a prekresleni menu Utility
//------------------------------------------------------------------------
void InitUtilsMenu()
{
  ShowHint("\0x0");
  Frame(12,10,36,14,16+7,0);
...
void RefreshSetMenu()
{
  ...
  MenuItems[ 3].Y=13; MenuItems[ 3].Item="Potlacit znaky     \0x0";
  MenuItems[ 4].Y=14; MenuItems[ 4].Item="Delay pro ASCII    \0x0";
  MenuItems[ 5].Y=15; MenuItems[ 5].Item="Delay pro HEX      \0x0";
  MenuItems[ 6].Y=16; MenuItems[ 6].Item="Ukoncovaci dialog  \0x0";
...
//------------------------------------------------------------------------
// Vykresleni ramecku a prekresleni menu Nastaveni
//------------------------------------------------------------------------
void InitSetMenu()
{
  ShowHint("\0x0");
  Frame(12,9,33,11+SetMenuItems,16+7,0);
  gotoxy(11,10); cprintf("Æµ");
  ...
  while(1)
  {
     for(I=0;I<5;I++)
     {
       ...
       if(MenuItems[I].Item[0]!=0)
       {
         ...
         if(AdrPort(I)!=0)
           cprintf(" %s / %3Xh ",MenuItems[I].Item,AdrPort(I));
         else
     ...
     switch(Scan)
     {
       case 62: if(Config.Port==4)
                HexToChar(Config.AdresaPortu);
                InputHEXLine(54,MenuItems[4].Y-1,3,16+15, HEX);
                HEX[0]=Vstup[1]; HEX[1]=Vstup[2];
                Config.CustomPortAdress=Config.AdresaPortu=CharToHex();

                HEX[0]=Vstup[0]; HEX[1]=0;
                Config.CustomPortAdress=CharToHex()*100 + Config.AdresaPortu;
...
void SetPort()
{
  ...
  while(1)
  {
     ...
     switch(STPPol)
     {
       case 0: ShowHint("Pocet stop bitu ( 0, 1, 2 )\0x0"); break;
...
void SetDelayASCII()
{
  ...
  Frame(35,13,49,15,16+7,0);
  gotoxy(34,14); cprintf("Æµ");
  gotoxy(37,13); cprintf(" Delay [ms] ");
  InitDownMenu("~Alt-X~ Exit  ~PgUp/PgDn//~ Zmenit  ~ENTER/ESC~ Ok");
  textattr(15);
  while(1)
  {
    gotoxy(36,14); cprintf("   %5d      ",Config.Delay_Ascii);
...
void SetDelayHEX()
{
  ...
  while(1)
  {
    gotoxy(36,15); cprintf("   %5d      ",Config.Delay_HEX);
...
{
  GetScr(1,33,9,54,17);
  Frame(35,10,54,16,16+7,0);
  gotoxy(34,13); cprintf("Æµ");
  InitDownMenu("~Alt-X~ Exit  ~PgUp/PgDn~ Zmenit");

  ResetMenuItems();
  MenuItems[ 0].Y=11; MenuItems[ 0].Item="#0Dh ( LF ) \0x0";
  MenuItems[ 1].Y=12; MenuItems[ 1].Item="#0Ah ( CR ) \0x0";
  MenuItems[ 2].Y=13; MenuItems[ 2].Item="#07h (BELL) \0x0";
  MenuItems[ 3].Y=14; MenuItems[ 3].Item="#08h (BCK)  \0x0";
  MenuItems[ 4].Y=15; MenuItems[ 4].Item="#09h (TAB)  \0x0";
  ...
  while(1)
  {
     ...
     switch(DCPol)
     {
       case 0: ShowHint("Povolit/Zakazat zobrazeni LF\0x0"); break;
       case 1: ShowHint("Povolit/Zakazat zobrazeni ENTER\0x0"); break;
       case 2: ShowHint("Povolit/Zakazat zobrazeni BELL\0x0"); break;
       case 3: ShowHint("Povolit/Zakazat zobrazeni Backspace\0x0"); break;
       case 4: ShowHint("Povolit/Zakazat zobrazeni Tab\0x0"); break;
     }
     GetKey();
     if((Scan==72) & (DCPol>0)) DCPol--;
     if((Scan==80) & (DCPol<4)) DCPol++;
     if(Scan==71) DCPol=0;
     if(Scan==79) DCPol=4;
...
{
  GetScr(1,33,9,54,17);
  Frame(35,15,49,18,16+7,0);
  gotoxy(34,16); cprintf("Æµ");
  InitDownMenu("~Alt-X~ Exit  ~ESC~ Zpet");

  ResetMenuItems();
  MenuItems[ 0].Y=16; MenuItems[ 0].Item="Nezobrazovat\0x0";
  MenuItems[ 1].Y=17; MenuItems[ 1].Item=" Zobrazovat \0x0";
  ...
  while(1)
  {
     ...
     {
       case 0: ShowHint("Pred ukoncenim terminalu se zobrazi potvrzovaci dialog"); break;
       case 1: ShowHint("Pri ukonceni terminalu se nezobrazi potvrzovaci dialog"); break;
     }
  }
  SetScr(1,33,9,54,17);
}
...
void Terminal()
{
  ...
  while(1)
  {
    ...
    if(CharReady())
    {
       Byte=FilterChar(inp(Config.AdresaPortu)); cprintf("%c",Byte);
    }
...
void TerminalHEX()
{
  ...
  while(1)
  {
    ...
    if(CharReady())
    {
       ...
       LoadCursorPos(1);
       HideTextCursor();
       HexToChar(inp(Config.AdresaPortu));
...
void Terminal2COM()
{
  GetScr(0,17,11,67,14);
  Frame(26,12,55,14,15,1);
  textattr(15);
  gotoxy(37,12); cprintf(" Warning ");
  gotoxy(28,13); cprintf("Feature not implemented yet");
  while(1)
  {
    GetKey(); if((Scan==1) | (Scan==28)) break;
  }
  SetScr(0,17,11,67,14);
}
...
void Nastaveni()
{
  ...
  while(1)
  {
     ...
     switch(SPol)
     {
       ...
       case 4: ShowHint("Nastaveni zpozdeni mezi jednotlivymi znaky pri vysilani bufferu ( ASCII )\0x0"); break;
       case 5: ShowHint("Nastaveni zpozdeni mezi jednotlivymi znaky pri vysilani bufferu ( HEX )\0x0"); break;
       case 6: ShowHint("Povoleni / Zakaz zobrazeni ukoncovaciho potvrzeni \0x0"); break;
     }
     ...
     if((Scan==28) | (Scan==77))
     {
        switch( SPol )
        {
          ...
          case 4:  SetDelayASCII(); break;
          case 5:  SetDelayHEX(); break;
          case 6:  MenuExitDialog(); break;
          default: break;
        }
     }
     if((Scan==45)&(Ascii==0)) Exit=1;
...
void Utility()
{
  ...
  while(1)
  {
     ...
     if((Scan==80) & (UPol<2)) UPol++;    // Down
     if(Scan==71) UPol=0;                 // Home
     if(Scan==79) UPol=2;                 // End
...
{
  #define DistelSetMenuItems 13
  ResetMenuItems();
  MenuItems[ 0].Y=6;  MenuItems[ 0].Item="Zpozdeni v [ms] pro odesilani mezi bajty      \0x0";
  MenuItems[ 1].Y=7;  MenuItems[ 1].Item="Zpozdeni v [ms] pro odesilani mezi zpravami   \0x0";
  MenuItems[ 2].Y=8;  MenuItems[ 2].Item="Pocet automatickeho odeslani zpravy    \0x0";
  MenuItems[ 3].Y=9;  MenuItems[ 3].Item="Maximalni pocet prichozich prikazu            \0x0";
  MenuItems[ 4].Y=10; MenuItems[ 4].Item="Timeout pro detekci zpravy                  \0x0";
  MenuItems[ 5].Y=11; MenuItems[ 5].Item="Pipnout pri prichodu zpravy                   \0x0";
  MenuItems[ 6].Y=12; MenuItems[ 6].Item="Pipnout pri odeslani zpravy                   \0x0";
  MenuItems[ 7].Y=13; MenuItems[ 7].Item="Typ pipnuti                  \0x0";
  MenuItems[ 8].Y=14; MenuItems[ 8].Item="Minimalni povolena hodnota adresy             \0x0";
  MenuItems[ 9].Y=15; MenuItems[ 9].Item="Maximalni povolena hodnota adresy             \0x0";
  MenuItems[10].Y=16; MenuItems[10].Item="Zpusob detekovani jednotlivych zprav  \0x0";
  MenuItems[11].Y=17; MenuItems[11].Item="Casove odesilani \0x0";
  MenuItems[12].Y=18; MenuItems[12].Item="Pocet byte v bufferu pro detekci zprav        \0x0";
  MenuItems[13].Y=19; MenuItems[13].Item="Maximalni povolena delka zpravy               \0x0";
  ...
  for(I=0;I<=DistelSetMenuItems;I++)
  {
    ...
    gotoxy(58,MenuItems[2].Y); cprintf("[%10.0f] ",(float) Config.DistelMaxAutoSend);
    ...
    switch(Config.DistelMSGDetectionType)
    {
      ...
      case 3: cprintf("Compare / T.+CRC "); break;
    }
...
void DistelSettings()
{
  ...
  SaveCursorPos(0);
  window(1,1,80,25);
  GetScr(2,13,4,70,9+DistelSetMenuItems);
  GetScr(3,0,24,79,24);
  InitDownMenu("~ESC/ENTER~ OK  ~Ctrl/Left/Right~ Change  ~Ctrl+D~ Default  ~Del/Ctrl+C~ Reset");
  Frame(14,5,70,7+DistelSetMenuItems,16+32+64,0);
  gotoxy(38,5); printf(" Nastaveni ");
  while(1)
  {
  ...
  Scan=0; Ascii=0;
  SetScr(2,13,4,70,9+DistelSetMenuItems);
  SetScr(3,0,24,79,24);
  LoadCursorPos(0);
}
...
//=================== Rutina preruseni od serialu ==========================
void interrupt SerialInterrupt()
{
  if(CharReady())
  {
    DistelBuf[BufIndex]=inp(Config.AdresaPortu);
    BufIndex++;
...
void ProtokolDistel()
{
  unsigned char Ch, Break, I, _Buffer, SelectedBuf;
  ...
  unsigned char SendByteIndex;
  unsigned char SendMsgIndex;
  unsigned char MsgExists;
  unsigned char CS, LN;
  unsigned char VBufSize;
  ...
  while(1)
  {
    if(kbhit())
    {
      ...
      if(_DistelAutoSend==0)
      {
        ...
        // Zobrazi nektere promenne
        if((Scan==23) & (Ascii==9))     // CTRL+L
        {
          gotoxy(1,1); cprintf("SetTimeout=%5d",SetTimeout);
          DetectTimeout=1;
        }
        ...
        if((Scan==143) & (Ascii==0))   // CTRL+NUM5
        {
          ...
          while(1)
          {
            ...
            */
            ...
            if(inp(0x2FD) & 1)
            {
              gotoxy(1,1); cprintf("Znak: %1c   ",inp(0x2F8));
...
//==================================================================
    if(Config.DistelMSGDetectionType==3)
    {
      if(Timeout>=0) Timeout--;

      // Zjisteni poctu byte v bufferu
      if(BufIndex > BufPos) VBufSize=(BufIndex-BufPos);
      else VBufSize=(sizeof(DistelBuf)-BufPos+BufIndex);

      // Je-li v bufferu prednastaveny pocet byte nebo dosel Timeout...
      if((VBufSize >= Config.DistelBytesForDetect) | (Timeout <= 0))
      {
        ...
        while(1)
        {
             ...
             (Config.DistelMaxMSGLength > LN))
          {
               ...
               (DistelBuf[BufPos]<=Config.DistelAdrMax))
            {
              ...
              {
                if((DistelBuf[BufPos]==Adr_D[PosY]) & (DistelBuf[BufPos+2]==Cmd_D[PosY]))
                {
                  MsgExists=1;
                  PocMsg_D[PosY]++;
                  RefreshSRMsgStatus(MsgSended,MsgReceived);
...
void main(unsigned char PocPar, char *ParamStr[])
{
  ...
  cprintf("Ulozeni puvodniho vektor preruseni serialu... ");
  OldInterrupt = getvect(0x1B);
  if(OldInterrupt) cprintf("Ok\n\r");
  else cprintf("Error\n\r");
  cprintf("Nastavuji vektor preruseni... ");
  //setvect(0x1B, SerialInterrupt());
  //setvect(0x1B, OldInterrupt);
  ...
  Pol=UPol=SNPol=NPol=SPol=DSPol=0;
  HEX[2]=0;
  while(1)
  {
     InitDownMenu("~Alt-X~ Exit                                          ");
     switch(Pol)
     {
       ...
       case 4: ShowHint("Znaky => Vstup. okno + => COM po zadani celeho cisla. Prijate zobraz."); break;
       case 5: ShowHint("Znaky => Vstup. okno + => COM po zadani celeho cisla. Prijate zobraz."); break;
       case 6: ShowHint("Konfigurace"); break;
       case 7: ShowHint("Ruzne pomucky"); break;
       case 8: ShowHint("Protokol pro test kotelen firmy Elitronic"); break;
       //case 9: ShowHint("Napoveda k programu"); break;
       case 9: ShowHint("Ukonceni programu"); break;
     }

     GetKey(); if((Scan==45)&(Ascii==0)) Exit=1;
     if((Scan==72) & (Pol>0)) Pol--;
     if((Scan==80) & (Pol<9)) Pol++;
     if(Scan==71) Pol=0;
     if(Scan==79) Pol=8;
       ...
       switch( Pol )
        {
          ...
          case  5: Terminal2COM(); break;
          case  6: Nastaveni(); break;
          case  7: Utility(); break;
          case  8: ProtokolDistel(); break;
          //case  9: Napoveda(); break;
          case  9: Exit=1; break;
          default: break;
        }
     }
